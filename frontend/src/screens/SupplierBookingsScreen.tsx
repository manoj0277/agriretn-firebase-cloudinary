...
const SupplierBookingsScreen: React.FC<SupplierViewProps> = ({ navigate }) => {
    ...
    const { bookings, markAsArrived, verifyOtpAndStartWork, completeWork } = useBooking();
    const { items } = useItem();
                ...
                return (
                    <div key={booking.id} className="bg-white dark:bg-neutral-700 p-4 rounded-lg border border-neutral-200 dark:border-neutral-600 mb-3">
                        ...
                        <div className="text-sm text-neutral-700 dark:text-neutral-300 space-y-1">
                             <p><strong>Date:</strong> {booking.date} at {booking.startTime}{booking.endTime ? ` - ${booking.endTime}` : (booking.estimatedDuration ? ` (Est. ${booking.estimatedDuration} hrs)`: '')}</p>
                             {booking.finalPrice && <p><strong>Payout:</strong> ₹{booking.finalPrice.toLocaleString()}</p>}
                        </div>
                        ...
                        <div className="text-right mt-4 border-t border-neutral-100 dark:border-neutral-600 pt-3 flex justify-end items-center flex-wrap gap-2">
                            ...
                            {booking.status === 'In Process' && (
                                <Button onClick={() => completeWork(booking.id)} className="w-auto px-4 py-1 text-sm bg-orange-500 hover:bg-orange-600">Complete Work</Button>
                            )}
                            {booking.status === 'Pending Payment' && (
                                <Button onClick={() => navigate({ view: 'PAYMENT', booking })} variant="secondary" className="w-auto px-4 py-1 text-sm">
                                    View Payment
                                </Button>
                            )}
