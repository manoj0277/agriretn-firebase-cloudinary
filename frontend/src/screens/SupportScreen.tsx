...
const SupportScreen: React.FC<SupportScreenProps> = ({ navigate, goBack }) => {
    ...
    const getAuthorName = (authorId: number) => {
        ...
        if (author?.role === UserRole.Admin) return t('supportTeam');
                    ...
                    {showContactForm ? (
                        <div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <Input label={t('fullName')} value={name} onChange={e => setName(e.target.value)} required readOnly={!!user} />
                                <Input label={t('email')} type="email" value={email} onChange={e => setEmail(e.target.value)} required readOnly={!!user}/>
                                <div>
                                <label className="block text-neutral-700 dark:text-neutral-300 text-sm font-bold mb-2">{t('content')}</label>
