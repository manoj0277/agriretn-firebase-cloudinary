...
const ItemCard: React.FC<ItemCardProps> = ({ item, onClick }) => {
    ...
    return (
        <div onClick={onClick} className="bg-white rounded-lg border border-neutral-200 shadow-sm hover:shadow-lg hover:border-primary/50 transition-all cursor-pointer overflow-hidden">
            <div>
                ...
                <div className="p-3">
                    <div className="flex justify-between items-center mb-1">
                        <div className="flex items-center text-xs text-neutral-500">
                         ...
                         <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${item.available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                            {item.available ? 'Available' : 'Booked'}
                        </span>
                    </div>
                    <h3 className="font-bold text-md text-neutral-900 leading-tight truncate mb-2">{item.name}</h3>
                    <div className="flex justify-between items-end">
                        <div>
                            {item.avgRating && item.avgRating > 0 ? (
                                <div className="flex items-center space-x-1">
                                    <StarRating rating={item.avgRating} />
                                    <span className="text-xs text-neutral-600">({item.avgRating.toFixed(1)})</span>
                                </div>
                            ) : (
                        ...
                        <div className="text-right">
                            ...
                            <p className="font-bold text-lg text-primary">₹{minPrice}<span className="text-xs font-normal text-neutral-500">/hr</span></p>
                        </div>
                    </div>
                </div>
            </div>
