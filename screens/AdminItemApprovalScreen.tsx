...
const AdminItemApprovalScreen: React.FC = () => {
    ...
    const [editData, setEditData] = useState<{ name: string; description: string; purposes: { name: string; price: number }[] }>({ name: '', description: '', purposes: [] });
    ...
    const getSupplierName = (ownerId: string | number) => {
        const supplier = allUsers.find(u => String(u.id) === String(ownerId));
    ...
    return (
        <div className="p-4 space-y-4">
            ...
            {previewItem && (
                ...
                >
                    <div
                        className="relative bg-white dark:bg-neutral-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden"
                    ...
                    >
                        ...
                        <div className="flex justify-between items-center p-4 border-b border-neutral-200 dark:border-neutral-700 flex-shrink-0">
                            <h3 className="font-bold text-lg text-neutral-800 dark:text-neutral-100">
                                {editMode ? 'Edit Item' : previewItem.name}
                            </h3>
                            <button
                                onClick={() => { setPreviewItem(null); setEditMode(false); }}
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {editMode ? (
                                ...
                                <div className="space-y-4">
                                    {/* Item Name */}
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">Item Name</label>
                                        <input
                                            type="text"
                                            value={editData.name}
                                            onChange={(e) => setEditData({ ...editData, name: e.target.value })}
                                            className="w-full font-bold text-lg text-neutral-800 dark:text-neutral-100 bg-neutral-100 dark:bg-neutral-700 px-3 py-2 rounded-lg border border-neutral-300 dark:border-neutral-600 focus:ring-2 focus:ring-primary"
                                    ...
                                    {/* Description */}
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">Description</label>
                                        <textarea
                                            value={editData.description}
                                            onChange={(e) => setEditData({ ...editData, description: e.target.value })}
                                            className="w-full text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-100 dark:bg-neutral-700 px-3 py-2 rounded-lg border border-neutral-300 dark:border-neutral-600 focus:ring-2 focus:ring-primary"
                                    ...
                                    {/* Prices */}
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">Prices</label>
                                        <div className="space-y-2 bg-neutral-50 dark:bg-neutral-900 p-3 rounded-lg">
                                            {editData.purposes.map((p, idx) => (
                                                <div key={idx} className="flex items-center gap-3">
                                                    <span className="text-sm font-medium min-w-[100px]">{p.name}:</span>
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <input
                                                            ...
                                                            value={p.price}
                                                            onChange={(e) => {
                                                                const newPurposes = [...editData.purposes];
                                                                newPurposes[idx] = { ...newPurposes[idx], price: parseFloat(e.target.value) || 0 };
                                                                setEditData({ ...editData, purposes: newPurposes });
                                                            }}
                                                            className="w-28 text-sm bg-white dark:bg-neutral-700 px-2 py-2 rounded border border-neutral-300 dark:border-neutral-600"
                                                        />
                                    ...
                                    {previewItem.images && previewItem.images.length > 0 && (
                                        <div>
                                            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">Images</label>
                                            <div className="flex gap-2 overflow-x-auto pb-2">
                                                {previewItem.images.map((img, idx) => (
                                                    <img
                                                        ...
                                                        className="h-24 w-32 object-cover rounded border border-neutral-200 dark:border-neutral-600"
                                                    />
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    <div className="bg-neutral-50 dark:bg-neutral-900 p-3 rounded-lg">
                                        <div className="flex gap-4 text-sm text-neutral-600 dark:text-neutral-400">
                                            ...
                                            <span className={`px-2 py-0.5 rounded text-xs font-semibold ${getStatusColor(previewItem.status)}`}>
                                    ...
                                    {previewItem.images && previewItem.images.length > 0 ? (
                                        <>
                                            <img
                                                ...
                                                className="max-h-[40vh] w-auto object-contain rounded-lg shadow-lg"
                                            />
                        ...
                        <div className="flex justify-between items-center p-4 border-t border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 flex-shrink-0">
                            {!editMode && (
                                <div className="flex gap-2 text-sm text-neutral-600 dark:text-neutral-400 flex-wrap">
                                    <span><strong>Supplier:</strong> {getSupplierName(previewItem.ownerId)}</span>
                                ...
                                {!editMode ? (
                                    ...
                                    <>
                                        ...
                                        <Button
                                            onClick={async () => {
                                                try {
                                                    const res = await fetch(`${API_URL}/items/${previewItem.id}`, {
                                                        method: 'PUT',
                                                        headers: { 'Content-Type': 'application/json' },
                                                        body: JSON.stringify({
                                                            name: editData.name,
