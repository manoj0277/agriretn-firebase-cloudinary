...
const FarmerBookingsScreen: React.FC<FarmerViewProps> = ({ navigate }) => {
    const { user, allUsers } = useAuth();
    const { bookings, cancelBooking, raiseDispute, startWork, completeBooking, makeFinalPayment } = useBooking();
    const { items } = useItem();
                        ...
                        <div key={booking.id} className="bg-white dark:bg-neutral-700 p-4 rounded-lg border border-neutral-200 dark:border-neutral-600 shadow-sm">
                            <div className="flex justify-between items-start">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-neutral-700 dark:text-neutral-300 text-sm">{booking.date} from {booking.startTime} - {booking.endTime}</p>
                                        ...
                                        {(booking.status === 'Confirmed' || booking.status === 'Completed' || booking.status === 'Pending Payment') && booking.finalPrice &&
                            ...
                            <div className="text-right mt-4 border-t border-neutral-100 dark:border-neutral-600 pt-3 flex justify-end items-center flex-wrap gap-x-4 gap-y-2">
                                ...
                                {booking.status === 'In Process' && (
                                    <Button onClick={() => { completeBooking(booking.id); navigate({ view: 'PAYMENT', booking, fromCompletion: true }); }} className="w-auto px-4 py-1 text-sm">Mark as Completed</Button>
                                )}
                                {booking.status === 'Pending Payment' && booking.finalPrice && booking.advanceAmount && (
